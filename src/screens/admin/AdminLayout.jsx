...
/* Содержимое сайдбара — переиспользуется и в десктопном <aside>, и внутри
   мобильного Drawer, чтобы не дублировать разметку/логику дважды. */
function AdminNavContent({ activeKey, badges, onNavigate }) {
  ...
  return (
    <>
      ...
      <nav style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '4px 12px 12px', display: 'flex', flexDirection: 'column', gap: 18 }}>
        <div>
          {top.map(item => <NavItem key={item.key} item={item} active={item.key === activeKey} onNavigate={onNavigate} />)}
        </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                {items.map(item => (
                  <div key={item.key} style={{ position: 'relative' }}>
                    <NavItem item={item} active={item.key === activeKey} onNavigate={onNavigate} />
  ...
  return createPortal(
    <div style={{ position: 'fixed', inset: 0, zIndex: 300, display: 'flex' }}>
      ...
      <div style={{
        ...
        <AdminNavContent activeKey={activeKey} badges={badges} onNavigate={onClose} />
      </div>
    </div>,
    document.body
  );
    ...
    <div className="admin-root" style={{ display: 'flex', height: '100%', overflow: 'hidden' }}>
      {!isCompact && (
        ...
        <aside style={{
          ...
          <AdminNavContent activeKey={activeKey} badges={badges} />
        </aside>
      )}
      {isCompact && drawerOpen && (
        <AdminMobileDrawer activeKey={activeKey} badges={badges} onClose={() => setDrawerOpen(false)} />
      )}
      ...
      <div style={{ flex: 1, minWidth: 0, height: '100%', display: 'flex', flexDirection: 'column' }}>
        <header style={{
          ...
          display: 'flex', alignItems: 'center', gap: isCompact ? 10 : 16, padding: isCompact ? '0 14px' : '0 28px',
          ...
          {isCompact && (
            <button
              ...
              style={{ width: 36, height: 36, flexShrink: 0, borderRadius: 9, border: '1px solid var(--border-subtle)', background: 'var(--bg-elev-1)', color: 'var(--text-3)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}
            >
              <Icon name="menu" size={17} />
            </button>
          )}
          <div style={{ minWidth: 0, overflow: 'hidden' }}>
            <div style={{ fontSize: isCompact ? 16 : 20, fontWeight: 600, letterSpacing: '-0.01em', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{adminNavByPath(pathname).label}</div>
            {!isCompact && <div style={{ fontSize: 12.5, color: 'var(--text-muted)', marginTop: 2 }}>{adminNavByPath(pathname).subtitle}</div>}
          </div>
          <span style={{ flex: 1 }} />
          {!isCompact && (
            <button
        ...
        <main className="ws-scroll" style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: isCompact ? '14px 14px 18px' : '18px 24px 24px' }}>
          <Outlet />
        </main>
      </div>
    </div>
